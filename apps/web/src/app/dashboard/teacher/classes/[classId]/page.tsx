...
export default function ClassDetailPage() {
  ...
  const classAvgScore =
    cls.memberStats.length > 0
      ? Math.round(
          cls.memberStats.reduce((acc, m) => acc + (m.avgScore || 0), 0) /
            cls.memberStats.length,
        )
      : 0;
    ...
    <div className="min-h-screen relative">
      ...
      <div className="max-w-6xl mx-auto space-y-6 pb-24 lg:pb-8 px-4 md:px-6 lg:px-8 pt-8">
            ...
            <div className="space-y-2 z-10">
              ...
              <p className="text-foreground-muted text-sm flex items-center gap-4">
                <span className="flex items-center gap-1.5">
                  <Users className="w-4 h-4" /> {cls.memberStats.length}{" "}
                  Students
                </span>
                <span className="flex items-center gap-1.5">
                  <BookOpen className="w-4 h-4" /> {totalAssignments}{" "}
                  Assignments
                </span>
              </p>
            </div>
            ...
            <div className="flex items-center gap-4 z-10 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 hide-scrollbar">
              ...
              <div className="bg-surface/50 border border-card-border p-4 rounded-xl min-w-[200px]">
                ...
                <div className="flex items-center gap-2">
                  <p className="text-2xl font-bold font-mono tracking-widest text-accent-light">
                    {inviteCode}
                  </p>
                  <button
                    onClick={handleCopy}
                  ...
                  >
                    <RefreshCw
                      className={`w-4 h-4 text-foreground-muted group-hover:text-foreground ${regenerateMutation.isPending ? "animate-spin" : ""}`}
                    />
                  </button>
                </div>
              </div>
        ...
        <motion.div
          ...
          className="flex pb-2 border-b border-card-border gap-6"
        >
          <button
          ...
          >
            ...
            {activeTab === "STUDENTS" && (
              <motion.div
                layoutId="clstab"
                className="absolute bottom-0 left-0 right-0 h-0.5 bg-accent-light rounded-t-full"
              />
            )}
          </button>
          <button
              ...
              {cls.memberStats.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <p className="text-sm text-foreground-muted mt-1 max-w-sm mb-6">
                    ...
                    <strong className="text-foreground">{inviteCode}</strong>{" "}
                  ...
                  <table className="w-full text-left text-sm">
                    <thead>
                      <tr className="border-b border-card-border bg-surface/30">
                        <th className="px-6 py-4 font-medium text-foreground-subtle">
                          Student
                        </th>
                        <th className="px-6 py-4 font-medium text-foreground-subtle">
                          Grade Lvl
                    ...
                    <tbody className="divide-y divide-card-border">
                      {cls.memberStats.map((student) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 font-semibold text-accent-light flex items-center gap-1">
                            <Sparkles className="w-3.5 h-3.5 text-yellow-400" />
                            {student.totalXPEarned} XP
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              {cls.assignments.length === 0 ? (
                <div className="card flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-accent text-white rounded-xl text-sm font-semibold hover:bg-accent-light transition-colors"
                  >
                    Create Assignment
                  </button>
                </div>
                ...
                cls.assignments.map((assignment) => (
                  <div
                    key={assignment.id}
                    className="card p-5 flex flex-col sm:flex-row cursor-default sm:items-center justify-between gap-4 group"
                  >
                    <div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <button
                        ...
                        className="px-4 py-2 text-sm font-medium bg-surface hover:bg-surface-hover border border-card-border rounded-lg transition-colors"
                      >
                        View
                      </button>
                    </div>
