    ...
    >
      <div className="space-y-2">
        ...
        <input
          ...
          placeholder="e.g. Summarize an article using AI"
        />
      </div>
      <div className="space-y-2">
        <label className="text-sm font-medium">
  ...
  return (
    ...
    >
      <p className="text-sm text-foreground-muted">
        Select the AI tool you'd like students to practice with. This is
        optional.
      </p>
      ...
      {selectedTool && (
        <div className="flex items-center gap-2 p-3 rounded-xl border border-accent/20 bg-accent-surface text-sm">
          <div
            ...
            style={{ backgroundColor: selectedTool.brandColor || "#6366f1" }}
          >
            {selectedTool.name[0]}
          </div>
          <span className="font-medium text-accent-light flex-1">
            {selectedTool.name}
          </span>
          <button
            onClick={() => setSelectedTool(null)}
            className="text-foreground-subtle hover:text-error text-xs"
          >
            Remove
          </button>
        </div>
        ...
        {tools?.map((tool) => (
          ...
          >
            <div
              ...
              style={{ backgroundColor: tool.brandColor || "#6366f1" }}
            >
              {tool.name[0]}
            </div>
  ...
  return (
    ...
    >
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <div
                ...
                style={{
                  backgroundColor: selectedTool.brandColor || "#6366f1",
                }}
              >
                {selectedTool.name[0]}
              </div>
      ...
      <div className="p-3.5 rounded-xl border border-success/20 bg-success/8 text-success text-sm flex items-center gap-2">
        ...
        This assignment will be published immediately and visible to all class
        members.
      </div>
    </motion.div>
  );
      ...
      <div className="max-w-2xl mx-auto pb-24 lg:pb-8 px-4 md:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-sm text-foreground-muted mt-1">
            Publish a new AI-powered assignment for your students
          </p>
        </motion.div>
        ...
        <div className="flex items-center gap-0 mb-8">
          {STEPS.map((s, i) => (
            <div key={s.id} className="flex items-center flex-1">
              ...
              {i < STEPS.length - 1 && (
                <div
                  className={`h-0.5 flex-1 transition-all ${i < step ? "bg-success" : "bg-card-border"}`}
                />
              )}
            </div>
          ))}
        ...
        <div className="mb-5 flex items-center gap-2">
          {(() => {
            ...
            return <Icon className="w-5 h-5 text-accent-light" />;
          })()}
        ...
        <div className="card p-6 min-h-[360px]">
          <AnimatePresence mode="wait">
            ...
            {step === 2 && (
              <ConfigStep dueDate={dueDate} setDueDate={setDueDate} />
            )}
            {step === 3 && (
              <PreviewStep
                title={title}
        ...
        {error && step !== 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="mt-4 flex items-center gap-2 p-3.5 rounded-xl bg-error/8 border border-error/20 text-error text-sm"
          >
            <AlertCircle className="w-4 h-4 flex-shrink-0" />
            {error}
          </motion.div>
        ...
        <div className="flex gap-3 mt-6">
          ...
          {step < STEPS.length - 1 && (
            <button
              onClick={handleNext}
              className="flex-1 flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-accent text-white font-semibold text-sm hover:bg-accent-light transition-all"
            >
              Next
