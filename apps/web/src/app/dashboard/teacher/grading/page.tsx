    ...
    <div className="min-h-screen relative">
      ...
      <div className="max-w-6xl mx-auto space-y-6 pb-24 lg:pb-8 px-4 md:px-6 lg:px-8">
        ...
        <motion.div
          ...
          className="pt-8 pb-4 flex flex-col md:flex-row md:items-end justify-between gap-4"
        >
          <div>
            <h1 className="font-display text-3xl font-bold tracking-tight">
              {isHindi ? "ग्रेडिंग केंद्र" : "Grading Center"}
            </h1>
            <p className="text-foreground-muted text-sm mt-1.5">
              {isHindi
                ? "छात्र सबमिशन की समीक्षा और ग्रेड करें"
          ...
          <div className="flex items-center gap-3 bg-surface/30 px-4 py-2 rounded-xl border border-white/5 shadow-sm">
            <div className="flex flex-col items-center px-3 border-r border-white/10">
              ...
              <span className="text-[10px] uppercase tracking-wider text-foreground-subtle font-medium">
                {isHindi ? "ग्रेड करना है" : "To Grade"}
              </span>
            </div>
            <div className="flex flex-col items-center px-3">
              <span className="text-xl font-bold text-success">
                {data?.averageGrade ? `${data.averageGrade.toFixed(1)}%` : "—"}
              </span>
              <span className="text-[10px] uppercase tracking-wider text-foreground-subtle font-medium">
                {isHindi ? "औसत वर्ग ग्रेड" : "Avg Class Grade"}
              </span>
            </div>
          </div>
        </motion.div>
          ...
          {filteredSubmissions.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <h3 className="font-display text-lg font-semibold">
                {isHindi ? "आप सभी काम कर चुके हैं!" : "You're all caught up!"}
              </h3>
              <p className="text-sm text-foreground-muted mt-1 max-w-sm">
                {statusFilter === "SUBMITTED"
                  ? isHindi
                    ? "अभी ग्रेड करने के लिए कोई सबमिशन नहीं है।"
                    : "There are no pending submissions that need grading right now."
                  : isHindi
                    ? "आपके फ़िल्टर से मेल खाने वाला कोई सबमिशन नहीं मिला।"
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-foreground-subtle mt-0.5">
                        {isHindi ? "द्वारा सबमिट" : "Submitted by"}{" "}
                  ...
                  <div className="mt-4 sm:mt-0 flex items-center gap-6 sm:w-auto w-full justify-between sm:justify-end">
                    ...
                    {sub.status === "SUBMITTED" ? (
                      <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-warning/10 text-warning text-xs font-medium border border-warning/20">
                        ...
                        {isHindi ? "ग्रेड चाहिए" : "Needs Grade"}
                      </div>
                    ) : (
