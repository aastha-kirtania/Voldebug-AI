  ...
  return (
    <Modal open={open} onClose={onClose} maxWidth="sm">
      <div className="text-center p-4">
        {/* Warning Icon Banner */}
        <div
          className={`mx-auto w-16 h-16 rounded-full flex items-center justify-center mb-6 border animate-pulse ${
            isCritical
        ...
        >
          <ShieldAlert
            className={`w-8 h-8 ${isCritical ? "text-red-500" : "text-error"}`}
          />
        </div>
        ...
        >
          ...
          <span>
            Blocked Category: {category} ({severity} Severity)
          </span>
        </div>
        ...
        <p
          className={`text-xs font-medium mb-6 ${isCritical ? "text-red-500" : "text-error"}`}
        >
          ⚠️ This event has been logged and sent to your teacher and school
          administration.
        </p>
